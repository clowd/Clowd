...
namespace PropertyTools.Wpf
{
    ...
    public class PropertyGrid : Control, IPropertyGridOptions
    {
        ...
        /// <summary>
        /// Identifies the <see cref="DescriptionIconTemplate"/> dependency property.
        /// </summary>
        public static readonly DependencyProperty DescriptionIconTemplateProperty =
            DependencyProperty.Register("DescriptionIconTemplate", typeof(DataTemplate), typeof(PropertyGrid));
        ...
        /// <summary>
        /// Gets or sets the description icon template.
        /// </summary>
        /// <value>The description icon.</value>
        public DataTemplate DescriptionIconTemplate
        {
            get
            {
                return (DataTemplate)this.GetValue(DescriptionIconTemplateProperty);
            }

            set
            {
                this.SetValue(DescriptionIconTemplateProperty, value);
            }
        }
        ...
        private void AddPropertyPanel(Panel panel, PropertyItem pi, object instance, Tab tab)
        {
            ...
            if (pi.TargetObject != null)
                propertyPanel.DataContext = pi.TargetObject;
            if (!pi.FillTab)
            {
                ...
                default:
                    {
                        ...
                        if (this.ShowDescriptionIcons && (this.DescriptionIcon != null || this.DescriptionIconTemplate != null))
                        {
                            if (!string.IsNullOrWhiteSpace(pi.Description))
                            {
                                FrameworkElement descriptionElement;
                                if (this.DescriptionIconTemplate != null)
                                {
                                    descriptionElement = new Border
                                    {
                                        Child = new ContentControl
                                        {
                                            ContentTemplate = DescriptionIconTemplate,
                                            HorizontalContentAlignment = HorizontalAlignment.Stretch,
                                            VerticalContentAlignment = VerticalAlignment.Stretch
                                        },
                                        // we nest this in a border control with a background set so that the tooltip
                                        // will show up for the entire rectangle that contains the icon template.
                                        Background = new SolidColorBrush(Color.FromArgb(1,0,0,0))
                                    };
                                }
                                else // if this.DescriptionIcon != null 
                                {
                                    descriptionElement = new Image
                                    {
                                        Source = this.DescriptionIcon,
                                        Stretch = Stretch.Uniform,
                                    };
                                }
                                descriptionElement.Focusable = false;
                                descriptionElement.HorizontalAlignment = this.DescriptionIconAlignment;
                                descriptionElement.VerticalAlignment = VerticalAlignment.Top;
                                descriptionElement.ToolTip = this.CreateToolTip(pi.Description);
                                descriptionElement.Margin = new Thickness(0, 4, 4, 4);

                                // we don't really want the description icon to increase the size of items so we
                                // set the max height of the icon to the desired height of the label minus the icon margin.
                                if(descriptionElement.DesiredSize == default(Size))
                                    labelPanel.Measure(new Size(Double.PositiveInfinity, Double.PositiveInfinity));
                                descriptionElement.MaxHeight = labelPanel.DesiredSize.Height 
                                    - (descriptionElement.Margin.Top + descriptionElement.Margin.Bottom);

                                labelPanel.Children.Add(descriptionElement);
                            }
                        }
                        else
                        {
