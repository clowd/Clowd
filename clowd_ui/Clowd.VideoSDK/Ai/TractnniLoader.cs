...
{
    /// <summary>
    /// Resolves the path to the <c>clowd_tractnni</c> inference binary. Resolution order: the
    /// caller-supplied resolver (the app passes the location its bundled binaries ship at), then
    /// the <c>CLOWD_TRACTNNI_PATH</c> environment variable. Clowd.VideoSDK deliberately has no
    ...
    /// </summary>
    public static class TractnniLoader
    {
        ...
        /// <summary>The full path of an existing <c>clowd_tractnni</c> binary, or null when none
