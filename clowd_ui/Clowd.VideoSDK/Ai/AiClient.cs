...
{
    /// <summary>
    /// One running <c>clowd_ai</c> child process: binary payload over stdin/stdout, logging
    ...
    public sealed class AiClient : IDisposable
    {
        ...
        public static AiClient Start(string exePath, IReadOnlyList<string> args)
        {
            ...
            return new AiClient(process);
        }

        private AiClient(Process process)
        {
            _process = process;
        ...
        public void ThrowIfFailed()
        {
            ...
            throw new InvalidOperationException(
                $"clowd_ai failed: {reason}." + (tail.Length > 0 ? Environment.NewLine + tail : ""));
        }
