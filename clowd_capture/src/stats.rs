...
impl Snapshot {
        ...
        T: Iterator<Item = &'a u64>,
    {
        ...
        scene.fill(
            Fill::NonZero,
            offset,
            Color::BLACK.multiply_alpha(0.75),
            None,
        ...
        for (i, sample) in samples.enumerate() {
            ...
            let color = match *sample {
                ..=16_667 => Color::rgba8(100, 143, 255, 255),
                ..=33_334 => Color::rgba8(255, 176, 0, 255),
                _ => Color::rgba8(220, 38, 127, 255),
            };
            scene.fill(
                Fill::NonZero,
                t * Affine::translate((left_margin_padding, (1 + labels.len()) as f64 * text_height)) * s,
                color,
                None,
                &bar,
            );
            ...
            .filter(|&&t| t < display_max)
        {
            ...
            text.add(
                ...
                Some(&Brush::Solid(Color::WHITE)),
                offset * Affine::translate((left_margin, (2. - y) * graph_max_height + thres_text_height_2)),
                &format!("{}", t),
            );
            scene.stroke(
...
pub fn draw_gpu_profiling(
    ...
    traverse_profiling(profiles, &mut |profile, stage| {
        if let TraversalStage::Enter = stage {
            if let Some(time) = &profile.time {
                let start_normalised = ((time.start - min) / total_time) * timeline_range_y + timeline_start_y;
                let end_normalised = ((time.end - min) / total_time) * timeline_range_y + timeline_start_y;
                ...
                let label = format!("{} - {:.30}", PaniclessSecondsFormatter(this_time), profile.label);
                scene.fill(
                    Fill::NonZero,
                    offset,
                    &Brush::Solid(color),
                    None,
                    &Rect::new(width * 0.31, cur_text_y - text_size as f64 * 0.7, width * 0.34, cur_text_y),
                );
                text.add(
                    scene,
                    None,
...
#[cfg(feature = "wgpu-profiler")]
fn traverse_profiling(profiles: &[GpuTimerQueryResult], callback: &mut impl FnMut(&GpuTimerQueryResult, TraversalStage)) {
