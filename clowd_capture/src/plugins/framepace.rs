...
fn get_display_refresh_rate(
    ...
    let new_frametime = match settings.limiter {
        ...
        Limiter::Off => {
            if settings.is_changed() {
                bevy::log::info!("Frame limiter disabled");
            }
            return;
        }
    };
    ...
    if let Ok(mut limit) = frame_limit.0.try_lock() {
        if new_frametime != *limit {
            bevy::log::info!("Frametime limit changed to: {:?}", new_frametime);
            *limit = new_frametime;
        }
    }
}
...
#[cfg(not(target_arch = "wasm32"))]
fn detect_frametime(winit: NonSend<WinitWindows>, windows: impl Iterator<Item = Entity>) -> Option<Duration> {
    let best_framerate = {
        windows
