...
// one of four modes; the predicates are checked in this order:
//
...
struct Instance {
    ...
    // (hover effect); >1.5 selects trail mode; <0 with corner_radius==0
    // selects dashed mode. aa_pad inflates the quad so the AA fringe on
    // rounded rects has room to fade. See the mode table at the top of
    // the file.
