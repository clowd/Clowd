namespace Clowd.ComServer.InteropServices
{
    ...
    [Flags]
    internal enum RegistrationClassContext
    {
        InProcessServer = 0x1,
    ...
    [Flags]
    internal enum RegistrationConnectionType
    {
        SingleUse = 0,
        MultipleUse = 1,
        MultiSeparate = 2,
    ...
    internal class RegistryServices
    {
        #region Constants
        ...
        public virtual bool RegisterAssembly(Assembly rtAssembly, AssemblyRegistrationFlags flags)
        {
            ...
            String strAsmName = rtAssembly.FullName;
            if (strAsmName == null)
                throw new InvalidOperationException("InvalidOperation_NoAsmName");

            // Retrieve the assembly codebase.
            String strAsmCodeBase = null;
            if ((flags & AssemblyRegistrationFlags.SetCodeBase) != 0)
            {
                strAsmCodeBase = rtAssembly.CodeBase;
            ...
            Type[] aTypes = GetRegistrableTypesInAssembly(rtAssembly);
            int NumTypes = aTypes.Length;

            String strAsmVersion = rtAssembly.GetName().Version.ToString();

            // Retrieve the runtime version used to build the assembly.
            String strRuntimeVersion = rtAssembly.ImageRuntimeVersion;
            ...
            Object[] aPIAAttrs = rtAssembly.GetCustomAttributes(typeof(PrimaryInteropAssemblyAttribute), false);
        ...
        [System.Security.SecurityCritical]  // auto-generated_required
        public virtual bool UnregisterAssembly(Assembly runtimeAssembly)
        {
            // Validate the arguments.
            if (runtimeAssembly == null)
                throw new ArgumentNullException("assembly");

            if (runtimeAssembly.ReflectionOnly)
                throw new InvalidOperationException("InvalidOperation_AsmLoadedForReflectionOnly");
            
            bool bAllVersionsGone = true;

            // Go through all the registrable types in the assembly and un-register them.
            Type[] aTypes = GetRegistrableTypesInAssembly(runtimeAssembly);
            int NumTypes = aTypes.Length;

            // Retrieve the assembly version
            String strAsmVersion = runtimeAssembly.GetName().Version.ToString();
            for (int cTypes = 0; cTypes < NumTypes; cTypes++)
            {
            ...
            Object[] aPIAAttrs = runtimeAssembly.GetCustomAttributes(typeof(PrimaryInteropAssemblyAttribute), false);
            int NumPIAAttrs = aPIAAttrs.Length;
            if (bAllVersionsGone)
            {
                for (int cPIAAttrs = 0; cPIAAttrs < NumPIAAttrs; cPIAAttrs++)
                    UnregisterPrimaryInteropAssembly(runtimeAssembly, (PrimaryInteropAssemblyAttribute)aPIAAttrs[cPIAAttrs]);
            }
        ...
        [System.Security.SecurityCritical]  // auto-generated_required
        public virtual Type[] GetRegistrableTypesInAssembly(Assembly runtimeAssembly)
        {
            // Validate the arguments.
            if (runtimeAssembly == null)
                throw new ArgumentNullException("assembly");
            ...
            Type[] aTypes = runtimeAssembly.GetExportedTypes();
            int NumTypes = aTypes.Length;
        ...
        [ResourceExposure(ResourceScope.Machine)]
        [ResourceConsumption(ResourceScope.Machine)]
        private void RegisterPrimaryInteropAssembly(Assembly runtimeAssembly, String strAsmCodeBase, PrimaryInteropAssemblyAttribute attr)
        {
            // Validate that the PIA has a strong name.
            if (runtimeAssembly.GetName().GetPublicKey().Length == 0)
                throw new InvalidOperationException("InvalidOperation_PIAMustBeStrongNamed");

            String strTlbId = "{" + Marshal.GetTypeLibGuidForAssembly(runtimeAssembly).ToString().ToUpper(CultureInfo.InvariantCulture) + "}";
            ...
            using (RegistryKey TypeLibRootKey = Registry.ClassesRoot.CreateSubKey(strTlbRootName))
            {
                ...
                using (RegistryKey TypeLibKey = TypeLibRootKey.CreateSubKey(strTlbId))
                {
                    ...
                    using (RegistryKey VersionSubKey = TypeLibKey.CreateSubKey(strVersion))
                    {
                        ...
                        VersionSubKey.SetValue("PrimaryInteropAssemblyName", runtimeAssembly.FullName);
                        if (strAsmCodeBase != null)
