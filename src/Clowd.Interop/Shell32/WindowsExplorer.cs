...
namespace Clowd.Interop.Shell32
{
    public static class WindowsExplorer
    {
        ...
        public static void ShowFilesOrFolders(string parentDirectory, ICollection<string> filenames)
        {
        ...
        {
            ShowFilesOrFolders((IEnumerable<string>)paths);
        }

        public static void ShowFilesOrFolders(IEnumerable<string> paths)
        {
            if (paths == null) throw new ArgumentNullException("paths");

            ShowFilesOrFolders(PathToFileSystemInfo(paths));
        }

        public static void ShowFilesOrFolders(IEnumerable<FileSystemInfo> paths)
        {
            if (paths == null) throw new ArgumentNullException("paths");
            ...
            foreach (var explorerWindowPaths in explorerWindows)
            {
                ...
                ShowFilesOrFolders(parentDirectory, explorerWindowPaths.Select(fsi => fsi.Name).ToList());
            }
        }
    }
}
