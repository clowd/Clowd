...
namespace Clowd.Helpers
{
    internal class NativeMethods2
    {
        ...
        public const uint SW_MINIMIZED = 2;
        public const uint SW_MAXIMIZED = 3;
        ...
        {
            RECT rect;
            var result = DwmGetWindowAttribute(handle, 9, out rect, Marshal.SizeOf(typeof(RECT)));
            rectangle = Rectangle.FromLTRB(rect.left, rect.top, rect.right, rect.bottom);
            return result >= 0;
        }
        ...
        {
            var z = 0;
            for (IntPtr h = hWnd; h != IntPtr.Zero; h = NativeMethods.GetWindow(h, NativeMethods.GETWINDOW_CMD.GW_HWNDPREV)) z++;
            return z;
        }
