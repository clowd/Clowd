...
/// <summary>
/// A color value that maintains both HSL and RGB representations in sync.
/// Ported from Clowd WPF (Clowd.Util.HslRgbColor).
/// </summary>
public sealed class HslRgbColor : INotifyPropertyChanged, IEquatable<HslRgbColor>
{
    ...
    private void UpdateHsl()
    {
        ...
        double hue = 0;
        double saturation = 0;
        double lightness = (c_max + c_min) / 2.0d;
        ...
        if (c_delta != 0)
        {
            if (lightness < 0.5d)
            {
                saturation = c_delta / (c_max + c_min);
            }
            else
            {
                saturation = c_delta / (2.0d - c_max - c_min);
            }
    ...
    private static byte ClampToByte(int v) => (byte)Math.Min(255, Math.Max(0, v));

    private static double Clamp(double v, double min = 0d, double max = 1d) => Math.Min(max, Math.Max(min, v));
