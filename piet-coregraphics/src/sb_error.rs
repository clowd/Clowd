    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::SizeOutOfRange {
                width,
                height,
            } => write!(f, "Surface size {width}x{height} out of range for backend.",),
...
impl From<SoftBufferError> for InitError {
...
impl<T, E: std::error::Error + 'static> SwResultExt<T> for Result<T, E> {
    fn swbuf_err(self, msg: impl Into<String>) -> Result<T, SoftBufferError> {
        self.map_err(|e| SoftBufferError::PlatformError(Some(msg.into()), Some(Box::new(LibraryError(e)))))
    }
}
