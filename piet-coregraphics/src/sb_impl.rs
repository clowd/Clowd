use crate::sb_error::{InitError, SoftBufferError};
...
use objc2::runtime::{AnyObject, Bool, MessageReceiver};
use objc2::{
    class, declare_class, msg_send, msg_send_id, mutability,
    runtime::{Class, Object, Protocol, Sel},
    sel, ClassType, DeclaredClass,
};
use objc2_foundation::{
    ns_string, CGPoint, MainThreadMarker, NSDictionary, NSKeyValueChangeKey, NSKeyValueChangeNewKey, NSKeyValueObservingOptions, NSNumber,
    NSObject, NSObjectNSKeyValueObserverRegistration, NSString, NSValue,
};
...
impl Observer {
    ...
    fn update(&self, key_path: Option<&NSString>, change: Option<&NSDictionary<NSKeyValueChangeKey, AnyObject>>) {
        ...
        if key_path == Some(ns_string!("contentsScale")) {
            ...
            let bounds = new
                .get_rect()
                .expect("new bounds value was not CGRect");
...
impl CGImpl {
    pub fn new(window_src: RawWindowHandle) -> Result<Self, InitError> {
        ...
        let root_layer = match window_src {
            ...
            _ => return Err(InitError::Unsupported),
        };
        ...
        unsafe {
            root_layer.addObserver_forKeyPath_options_context(
                ...
                NSKeyValueObservingOptions::NSKeyValueObservingOptionNew | NSKeyValueObservingOptions::NSKeyValueObservingOptionInitial,
                ptr::null_mut(),
            );
            root_layer.addObserver_forKeyPath_options_context(
                &observer,
    ...
    // pub fn get_context(&mut self) -> Result<&mut CGContextRef, SoftBufferError> {
    //     let cgcontext_ptr: *mut std::ffi::c_void = unsafe { self.layer.0.send_message(sel!(context), ()) };
    //     if cgcontext_ptr.is_null() {
    //         return Err(SoftBufferError::PlatformError(
    //             Some("No CGContext found in CALayer".to_string()),
    //             None,
    //         ));
    //     }

    //     let cgcontext = unsafe { CGContextRef::from_ptr_mut(cgcontext_ptr as *mut _) };
    //     Ok(cgcontext)
    // }
    ...
    pub fn present_context(&mut self, context: CGContext) -> Result<(), SoftBufferError> {
...
//     fn context(&mut self) -> &mut CGContext {
//         &mut self.context
//     }
...
//         let image = self.context().create_image().unwrap();
//         let contents = unsafe { (image.as_ptr() as *mut AnyObject).as_ref() };
