...
namespace Clowd.Shared
{
    public static class MimeAssistant
    {
        ...
        public static string GetMimeFromExtension(string extension)
        {
            ...
            {
                var mimes = from line in _mimeList.Split(new[] { '\n' }, StringSplitOptions.RemoveEmptyEntries)
                ...
                _mimeDictionary = mimes.ToDictionary(mime => mime.Mime, mime => mime.Extensions, StringComparer.InvariantCultureIgnoreCase).ToList();
            }

            var item = _mimeDictionary.FirstOrDefault(mime => mime.Value.Contains(extension, StringComparer.InvariantCultureIgnoreCase));
            if (item.Equals(default(KeyValuePair<string, string[]>)))
                return null;
            return item.Key;
        }

        //mime and extension are tab separated.
        private const string _mimeList =
